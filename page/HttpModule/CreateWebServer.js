...
const server= http.createServer((req,res)=>{            //inside createserver call back function pass where 2 parameter pass- req,res
    ...
    if(req.url === "/contact"){
        res.setHeader("Content-Type","text/plain");  // .setHeader() is me property define kar te hai Pascal case m- Content-Type , us ka type
        res.write("thx for contact");  //data write for response  (here we send plain text)
        res.end();     //data send
    }

    if(req.url === "/source-code"){
        res.setHeader("Content-Type","text/html");  // .setHeader() is me property define kar te hai Pascal case m- Content-Type , us ka type
...
//^ server create for listen request of the client  (2 argument pass - PORT ,callback)
server.listen(PORT,()=>{
...
//*syntax in terminal-        node --watch ./filename
